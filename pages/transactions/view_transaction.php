<?  //header('Content-type: text/html; charset=windows-1251');
	...
	function all_transactions() {
		echo "<h1 class='title'>Все Передачи Товаров</h1>";
		$page = $GLOBALS["page"];
...
<title><? if (isset($transaction_id)) { $trans = get_transaction($transaction_id); echo $trans['title'];} else { echo "Все Передачи Товаров"; }?></title>
...
if (!isset($transaction_id)) {
	...
	if ($transactionRes&&(mysqli_num_rows($transactionRes)>0)) {
		...
		if ($tTitle=="") {
			$tTitle = "Транзакция #".$transaction_id;
		}
?>			
            <h1 class="font-xl"><a target="_blank" href="print.php?transaction_id=<? echo $transaction_id;?>">
...
<?
		...
		echo "<div class='clear '></div><br><br><br><p class='two_third first font-large'>".make_link("view_transaction.php","","Все Транзакции")."</p>";
	}
	else {
		all_transactions();
	}
