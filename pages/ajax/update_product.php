...
<?
			...
			if (isset($name)&&isset($category_id)&&isset($price)) {
				/* all ok -> sqlquery */
				...
				if ($result) {
					$error = show_error("Изменения были удачно сохранены!", "success", 0);
					$footer_links = "<p class='clear'>
										<a class='fl_left' href=''.$root.'view_product.php?id=".$id."'>&laquo; Назад</a>
										<a class='fl_right' href=''.$root.'index.php'>На Главную &raquo;</a>
									</p>";
								
					echo "<meta http-equiv='refresh' content='0;URL=view_product.php?id=".$id."'>";
				}
				else {
					$mysql_error = mysqli_error($db);
					$error =  show_error("Изменения не были сохранены! <br> <br><strong>причина</strong>:<br>$mysql_error", "error", 0);
