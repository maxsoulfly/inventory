...
<?
	if (!isset($category_id) || !category_exists($category_id, $db)){
		...
		$cat_res = mysqli_query ($db, "SELECT * FROM locations");
		...
		if (mysqli_num_rows($cat_res)>0) {
			...
			do{
				...
				echo "<li ><a class='full_width' href='../categories/edit_category.php?id=" .$cat_row['id']."' title='".$cat_row['name']."'>".$cat_row['name']."</a></li>";
		...
		$cat_res = mysqli_query ($db, "SELECT * FROM locations WHERE id = $category_id");
        $cat_row = "";
		if (!$cat_res) {
			echo "<p>Ошибка при получении данных из БД. Пожалуйста, сообщите администратору по support@freeurmind.net <br> <STRONG> Код ошибки:</strong></p>";
			exit(mysqli_error($db));
...
?>
      	<h1><em class="icon-pencil"></em> Редактор Названия Филиала</h1>
        <form action="../categories/update_category.php" method="post" name="edit_category">
